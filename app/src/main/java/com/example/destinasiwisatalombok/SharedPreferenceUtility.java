...
public class SharedPreferenceUtility {
     ...
     */
    public static List<Wisata> getAllFavorite(Context ctx) {
        String jsonString = getSharedPref(ctx).getString(FAVOR_KEY, null);
        List<Wisata> trs = new ArrayList<Wisata>();
        if (jsonString != null) {
            ...
            try {
                ...
                for (int i=0;i<jsonArray.length();i++) {
                    ...
                    trs.add(Wisata.fromJSONObject(obj));
                }
    ...
    /*
        ...
        Perhatikan bahwa data disimpan dalam format JSON String
     */
    private static void saveAllFavorite(Context ctx, List<Wisata> trs) {
     ...
     */
    public static void addFavorite(Context ctx, Wisata tr) {
        List<Wisata> trs = getAllFavorite(ctx);
        trs.add(tr);
        saveAllFavorite(ctx,trs);
    }

    public static void editFavorite(Context ctx, Wisata tr) {
        List<Wisata> trs = getAllFavorite(ctx);
        for (Wisata tre:trs) {
